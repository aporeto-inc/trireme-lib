...
type mockedMethods struct {
	...
	LaunchProcessMock func(string, int, rpcwrapper.RPCClient, string, string, string, uint16, uint16) error
...
type TestProcessManager interface {
	...
	MockLaunchProcess(t *testing.T, impl func(string, int, rpcwrapper.RPCClient, string, string, string, uint16, uint16) error)
...
func (m *testProcessMon) MockLaunchProcess(t *testing.T, impl func(string, int, rpcwrapper.RPCClient, string, string, string, uint16, uint16) error) {
...
func (m *testProcessMon) LaunchProcess(contextID string, refPid int, rpchdl rpcwrapper.RPCClient, processname string, statssecret string, procMountPoint string, networkQueue uint16, appQueue uint16) error {
	if mock := m.currentMocks(m.currentTest); mock != nil && mock.LaunchProcessMock != nil {
		return mock.LaunchProcessMock(contextID, refPid, rpchdl, processname, statssecret, procMountPoint, networkQueue, appQueue)
