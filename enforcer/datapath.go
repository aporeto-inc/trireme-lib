...
func NewDatapathEnforcer(
	...
	d := &datapathEnforcer{
		...
		networkConnectionTracker: cache.NewCacheWithExpirationNotifier(time.Second*20, TCPConnectionExpirationNotifier),
		appConnectionTracker:     cache.NewCacheWithExpirationNotifier(time.Second*20, TCPConnectionExpirationNotifier),
		contextConnectionTracker: cache.NewCacheWithExpiration(time.Second * 20),
		sourcePortCache:          cache.NewCacheWithExpiration(time.Second * 20),
		destinationPortCache:     cache.NewCacheWithExpiration(time.Second * 20),
		filterQueue:              filterQueue,
