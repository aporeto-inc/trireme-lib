...
func New(context uint64, bytes []byte, mark string) (packet *Packet, err error) {
	...
	if len(p.Buffer) < TCPOptionPos {
		log.WithFields(log.Fields{
			"package":      "packet",
			"BufferLength": len(p.Buffer),
			"OptionPos":    TCPOptionPos,
		}).Error("Packet length is smaller then minimum size")
	} else {
...
func (p *Packet) TCPDataStartBytes() uint16 {
	return p.l4BeginPos + uint16(p.L4TCPPacket.tcpDataOffset)*TCPWordSize
}
...
func (p *Packet) TCPDataDetach(optionLength uint16) (err error) {
	...
	p.FixupIPHdrOnDataModify(p.IPTotalLength, p.IPTotalLength-uint16(len(p.L4TCPPacket.tcpData)))
	return
}

// FixupTCPHdrOnTCPDataAttach modifies the TCP header fields and checksum
func (p *Packet) FixupTCPHdrOnTCPDataAttach(tcpData []byte) {
	...
	delta := p.computeTCPChecksumDelta([]byte{}, uint16(len([]byte{})), tcpData, uint16(len(tcpData)))
	a := uint32(-p.L4TCPPacket.TCPChecksum) + delta
	a = a + (a >> 16)
	p.L4TCPPacket.TCPChecksum = -uint16(a)
	//20 is the fixed length header

	p.L4TCPPacket.tcpDataOffset = uint8((20 + len(p.L4TCPPacket.tcpOptions)) / 4)
...
func (p *Packet) TCPDataAttach(tcpData []byte) (err error) {

	log.WithFields(log.Fields{
		"package": "packet",
	}).Debug("TCP data attach")

	if err = p.tcpDataAttach(tcpData); err != nil {
		log.WithFields(log.Fields{
			"package": "packet",
			"error":   err.Error(),
	...
	// our option last.
	newPacketLength := uint16(len(p.L4TCPPacket.tcpData) + len(p.Buffer) + len(p.L4TCPPacket.tcpOptions))
	// IP Header Processing
	p.FixupIPHdrOnDataModify(p.IPTotalLength, newPacketLength)

	// TCP Header Processing
	p.FixupTCPHdrOnTCPDataAttach(tcpData)
