...
func TestCreateAndVerifyPSK(t *testing.T) {
	Convey("Given a JWT valid engine with pre-shared key ", t, func() {
		...
		Convey("Given a signature request for a normal packet", func() {
			...
			lclaims, ok1 := recoveredClaims.T.GetUnique("label1")
			dclaims, ok2 := recoveredClaims.T.GetUnique("label1")
			So(ok1, ShouldBeTrue)
			So(ok2, ShouldBeTrue)
...
func TestCreateAndVerifyPKI(t *testing.T) {
	Convey("Given a JWT valid engine with a PKI  key ", t, func() {
		...
		Convey("Given a signature request that hits the cache ", func() {
			...
			lclaims1, ok1 := recoveredClaims1.T.GetUnique("label1")
			dclaims1, ok2 := recoveredClaims1.T.GetUnique("label1")
			So(ok1, ShouldBeTrue)
			So(ok2, ShouldBeTrue)
			So(lclaims1, ShouldResemble, dclaims1)
			lclaims2, ok3 := recoveredClaims2.T.GetUnique("label1")
			dclaims2, ok4 := recoveredClaims2.T.GetUnique("label1")
			So(ok3, ShouldBeTrue)
			So(ok4, ShouldBeTrue)
