...
func Test_NewRemoteEnforcer(t *testing.T) {
	...
	Convey("When I try to retrieve rpc server handle", t, func() {
		...
		Convey("When I try to create new server with no env set", func() {
			...
			server, err := newRemoteEnforcer(ctx, cancel, nil, rpcHdl, "mysecret", statsClient, collector, debugClient, counterclient, dnsreportclient, tokenclient, zap.Config{}, policy.EnforcerMapping, semver.Version{})
...
func TestInitEnforcer(t *testing.T) {
	...
	Convey("When I try to retrieve rpc server handle", t, func() {
		...
		Convey("When I try to create new server with env set", func() {
			...
			server, err := newRemoteEnforcer(ctx, cancel, service, rpcHdl, secret, mockStats, mockCollector, mockDebugClient, mockCounterClient, mockDNSReportClient, mockTokenClient, zap.Config{}, policy.EnforcerMapping, semver.Version{})
			So(err, ShouldBeNil)
