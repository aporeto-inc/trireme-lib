...
func (d *Datapath) processNetworkPacketsFromNFQ(p *nfqueue.NFPacket) {
	...
	if netPacket.IPProto() == packet.IPProtocolTCP {
		...
		p.QueueHandle.SetVerdict2(uint32(p.QueueHandle.QueueNum), v, uint32(p.Mark&^(NFQueueMask|NFSetMarkMask)), uint32(copyIndex), uint32(p.ID), buffer)
	} else {
		p.QueueHandle.SetVerdict2(uint32(p.QueueHandle.QueueNum), v, uint32(p.Mark&^(NFQueueMask|NFSetMarkMask)), uint32(len(netPacket.GetBuffer(0))), uint32(p.ID), netPacket.GetBuffer(0))
	}
...
func (d *Datapath) processApplicationPacketsFromNFQ(p *nfqueue.NFPacket) {
	...
	if processError != nil {
		...
		length := uint32(len(p.Buffer))
		buffer := p.Buffer
		p.QueueHandle.SetVerdict2(uint32(p.QueueHandle.QueueNum), 0, uint32(p.Mark&^(NFQueueMask|NFSetMarkMask)), length, uint32(p.ID), buffer)
	...
	if appPacket.IPProto() == packet.IPProtocolTCP {
		...
		p.QueueHandle.SetVerdict2(uint32(p.QueueHandle.QueueNum), 1, uint32(p.Mark&^(NFQueueMask|NFSetMarkMask)), uint32(copyIndex), uint32(p.ID), buffer)

	} else {
		p.QueueHandle.SetVerdict2(uint32(p.QueueHandle.QueueNum), 1, uint32(p.Mark&^(NFQueueMask|NFSetMarkMask)), uint32(len(appPacket.GetBuffer(0))), uint32(p.ID), appPacket.GetBuffer(0))
	}
