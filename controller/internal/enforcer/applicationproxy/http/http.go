...
func (p *Config) processNetRequest(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := p.localIPs[originalDestination.IP.String()]; !ok {
		...
		if err != nil || action.Action.Rejected() {
			...
			http.Error(w, fmt.Sprintf("Access to downstream denied by network policy to downstream IP: %s", originalDestination.IP.String()), http.StatusNetworkAuthenticationRequired)
			return
		}
