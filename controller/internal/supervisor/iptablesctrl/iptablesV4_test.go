...
var (
	expectedGlobalMangleChainsV4 = map[string][]string{
		...
		"TRI-App": {
			...
			"-j TRI-UID-App",
			"-p tcp -m set --match-set TRI-v4-TargetTCP dst -m tcp --tcp-flags SYN,ACK SYN,ACK -j MARK --set-mark 6488064/0xfffffc00",
		...
		"TRI-Net": {
			...
			"-m set --match-set TRI-v4-TargetTCP src -p tcp --tcp-flags ALL ACK -m tcp --tcp-option 34 -m mark --mark 1/0x3ff -j NFQUEUE --queue-num 20",
			"-m set --match-set TRI-v4-TargetTCP src -p tcp --tcp-flags ALL ACK -m tcp --tcp-option 34 -m mark --mark 2/0x3ff -j NFQUEUE --queue-num 21",
			"-m set --match-set TRI-v4-TargetTCP src -p tcp --tcp-flags ALL ACK -m tcp --tcp-option 34 -m mark --mark 3/0x3ff -j NFQUEUE --queue-num 22",
			"-m set --match-set TRI-v4-TargetTCP src -p tcp --tcp-flags ALL ACK -m tcp --tcp-option 34 -m mark --mark 4/0x3ff -j NFQUEUE --queue-num 23",
			"-m connmark --mark 61167 -j ACCEPT",
			"-m connmark --mark 61166 -p tcp ! --tcp-flags SYN,ACK SYN,ACK -j ACCEPT",
			"-j TRI-UID-Net",
	...
	expectedMangleAfterPUInsertV4 = map[string][]string{
		...
		"TRI-Pid-App": {
			"-m cgroup --cgroup 10 -m comment --comment PU-Chain -j MARK --set-mark 655360/0xfffffc00",
			"-m mark --mark 655360/0xfffffc00 -m comment --comment PU-Chain -j TRI-App-pu1N7uS6--0",
		},
		"TRI-Pid-Net": {
	...
	expectedMangleAfterPUUpdateV4 = map[string][]string{
		...
		"TRI-Pid-App": {
			"-m cgroup --cgroup 10 -m comment --comment PU-Chain -j MARK --set-mark 655360/0xfffffc00",
			"-m mark --mark 655360/0xfffffc00 -m comment --comment PU-Chain -j TRI-App-pu1N7uS6--1",
		},
		"TRI-Pid-Net": {
