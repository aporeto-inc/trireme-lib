...
var (
	...
	expectedMangleAfterPUInsertV4 = map[string][]string{
		...
		"TRI-App-pu1N7uS6--0": {
			...
			"-p tcp -m tcp -m set --match-set TRI-v4-TargetTCP dst --tcp-flags SYN,ACK SYN -j NFQUEUE --queue-balance 0:3",
			"-p tcp -m tcp -m set --match-set TRI-v4-TargetTCP dst --tcp-flags SYN,ACK ACK -j NFQUEUE --queue-balance 4:7",
