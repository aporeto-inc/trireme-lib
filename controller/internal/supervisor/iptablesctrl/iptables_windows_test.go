...
func Test_WindowsConfigureRulesV4(t *testing.T) {
	...
	Convey("Given a valid instance", t, func() {
		...
		So(a.filters["HostPU-OUTPUT"], ShouldContainKey, "-p tcp -m set --match-set TRI-v4-TargetTCP dstIP -j NFQUEUE -j MARK 10")
		...
		So(a.filters["HostPU-OUTPUT"], ShouldContainKey, "-p udp -m set --match-set TRI-v4-TargetUDP dstIP -j NFQUEUE -j MARK 10")
		...
		So(a.filters["HostPU-INPUT"], ShouldContainKey, "-p tcp -m set --match-set TRI-v4-TargetTCP srcIP -j NFQUEUE -j MARK 10")
		So(a.filters["HostPU-INPUT"], ShouldContainKey, "-p udp -m set --match-set TRI-v4-TargetUDP srcIP -m string --string n30njxq7bmiwr6dtxq --offset 2 -j NFQUEUE -j MARK 10")
