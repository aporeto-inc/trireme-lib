...
func TestAddChainRules(t *testing.T) {
	...
	Convey("Given an iptables controller for LocalContainer", t, func() {
		...
		Convey("When I add the chain rules and they succeed", func() {
			...
			err := i.addChainRules("appchain", "netchain", "0", "100", "100", "", "", "5000", "proxyPortSet", false)
			So(err, ShouldBeNil)
		})
	...
	Convey("Given an iptables controller for LocalServer", t, func() {
		...
		Convey("When I add the chain rules for hostmode and they succeed", func() {
			iptables.MockAppend(t, func(table string, chain string, rulespec ...string) error {
				return nil
			})
			err := i.addChainRules("appchain", "netchain", "0", "100", "100", "", "", "5000", "proxyPortSet", true)
			So(err, ShouldBeNil)
		})
		...
		Convey("When I add the chain rules and the appPacketIPTableContext fails in hostmode ", func() {
			iptables.MockAppend(t, func(table string, chain string, rulespec ...string) error {
				if table == i.appPacketIPTableContext {
					return errors.New("error")
				}
		...
		Convey("When i add chain rules with non-zero uid and port 0", func() {
			...
			err := i.addChainRules("appchain", "netchain", "0", "0", "0", "1001", "", "5000", "proxyPortSet", false)
			So(err, ShouldBeNil)
		...
		Convey("When i add chain rules with non-zero uid and port 0 rules are added to the UID Chain", func() {
			...
			err := i.addChainRules("appchain", "netchain", "80", "0", "0", "1001", "", "5000", "proxyPortSet", false)
			So(err, ShouldBeNil)
...
func TestAddPacketTrap(t *testing.T) {
	...
	Convey("Given an iptables controller, when I test addPacketTrap for Local Server", t, func() {
		...
		Convey("When I add the packet trap rules and they succeed for hostpu", func() {
			iptables.MockAppend(t, func(table string, chain string, rulespec ...string) error {
				return nil
			})
			err := i.addPacketTrap("appchain", "netchain", []string{"172.17.0.0/24"}, true)
			Convey("I should get no error", func() {
				So(err, ShouldBeNil)
			})
		})
		...
		Convey("When I add the packet trap rules and the appPacketIPTableContext fails for hostpu ", func() {
			iptables.MockAppend(t, func(table string, chain string, rulespec ...string) error {
				if table == i.appPacketIPTableContext {
					return errors.New("error")
				}
		...
		Convey("When I add the packet trap rules and the netPacketIPtableContext fails for hostpu ", func() {
			iptables.MockAppend(t, func(table string, chain string, rulespec ...string) error {
				if table == i.netPacketIPTableContext {
					return errors.New("error")
				}
	...
	Convey("Given an iptables controller, when I test addPacketTrap for sidecar container", t, func() {
		...
		Convey("When I add the packet trap rules and the appPacketIPTableContext fails for host pu ", func() {
			iptables.MockAppend(t, func(table string, chain string, rulespec ...string) error {
				if table == i.appPacketIPTableContext {
					return errors.New("error")
				}
...
func TestDeleteChainRules(t *testing.T) {
	...
	Convey("Given an iptables controller", t, func() {
		...
		Convey("When I delete the chain rules and it succeeds", func() {
			...
			err := i.deleteChainRules("appchain", "netchain", "0", "100", "100", "", "", "5000", "proxyPortSetName", false)
			So(err, ShouldBeNil)
		})
		...
		Convey("When I delete the chain rules and it fails in hostmode in hostmode", func() {
			...
			err := i.deleteChainRules("appchain", "netchain", "0", "100", "100", "", "", "5000", "proxyPortSetName", true)
			So(err, ShouldBeNil)
