...
var (
	expectedGlobalMangleChainsV6 = map[string][]string{
		...
		"TRI-App": {
			...
			"-p tcp -m set --match-set TRI-v6-TargetTCP dst -j HMARK --hmark-tuple dst,dport,src,sport --hmark-offset 0x1 --hmark-rnd 0xdeafbee1 --hmark-mod 4",
			"-p udp -m set --match-set TRI-v6-TargetUDP dst -j HMARK --hmark-tuple dst,dport,src,sport --hmark-offset 0x1 --hmark-rnd 0xdeafbee1 --hmark-mod 4",
			"-j TRI-UID-App",
			"-p tcp -m set --match-set TRI-v6-TargetTCP dst -m tcp --tcp-flags SYN,ACK SYN,ACK -j MARK --set-mark 0/0xfffffc00",
		...
		"TRI-Net": {
			"-j TRI-Prx-Net",
			"-p udp -m set --match-set TRI-v6-TargetUDP src -j HMARK --hmark-tuple src,sport,dst,dport --hmark-offset 0x1 --hmark-rnd 0xdeafbee1 --hmark-mod 4",
			...
			"-p tcp -m set --match-set TRI-v6-TargetTCP src -j HMARK --hmark-tuple src,sport,dst,dport --hmark-offset 0x1 --hmark-rnd 0xdeafbee1 --hmark-mod 4",
			...
			"-m set --match-set TRI-v6-TargetTCP src -p tcp --tcp-flags ALL ACK -m tcp --tcp-option 34 -m mark --mark 4/0x3ff -j NFQUEUE --queue-num 24",
			"-m connmark --mark 61167 -j ACCEPT",
			"-m connmark --mark 61166 -p tcp ! --tcp-flags SYN,ACK SYN,ACK -j ACCEPT",
