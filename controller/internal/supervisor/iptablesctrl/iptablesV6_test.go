...
var (
	...
	expectedMangleAfterPUInsertV6 = map[string][]string{
		...
		"TRI-Net-pu1N7uS6--0": {
			...
			"-s ::/0 -m state --state NEW -j NFLOG --nflog-group 11 --nflog-prefix 12149274478354645901:6",
			"-s ::/0 -m state ! --state NEW -j NFLOG --nflog-group 11 --nflog-prefix 12149274478354645901:10",
			"-s ::/0 -j DROP",
		},
		...
		"TRI-App-pu1N7uS6--0": {
			...
			"-p tcp -m state --state ESTABLISHED -m comment --comment TCP-Established-Connections -j ACCEPT",
			"-d ::/0 -m state --state NEW -j NFLOG --nflog-group 10 --nflog-prefix 12149274478354645901:6",
			"-d ::/0 -m state ! --state NEW -j NFLOG --nflog-group 10 --nflog-prefix 12149274478354645901:10",
			"-d ::/0 -j DROP",
		},
	}
