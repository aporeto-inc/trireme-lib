...
var (
	...
	expectedMangleAfterPUInsert = map[string][]string{
		...
		"TRI-Net-pu1N7uS6--0": {
			...
			"-p udp -m set --match-set TRI-TargetUDP src --match limit --limit 1000/s -j NFQUEUE --queue-balance 16:19",
		...
		"TRI-App-pu1N7uS6--0": {
			...
			"-p tcp -m tcp --tcp-flags SYN,ACK SYN -m set --match-set TRI-TargetTCP dst -j NFQUEUE --queue-balance 0:3",
			"-p tcp -m tcp --tcp-flags SYN,ACK ACK -m set --match-set TRI-TargetTCP dst -j NFQUEUE --queue-balance 4:7",
