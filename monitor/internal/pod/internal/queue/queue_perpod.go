...
// ensure PerPodPolicyEngineQueue implements PolicyEngineQueue
var _ PolicyEngineQueue = &PerPodPolicyEngineQueue{}
...
func (q *PerPodPolicyEngineQueue) Enqueue(ctx context.Context, ev *PolicyEngineEvent) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	case q.queue <- ev:
